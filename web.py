...
# --- 2. DISEÑO CON CONTRASTE DE COLORES (BLANCO SOBRE DORADO) ---
...
st.markdown("""
    ...
    /* TITULOS EN BLANCO PARA QUE SE NOTEN (CAMBIO SOLICITADO) */
    .panel-header { 
    ...
    .price-main { color: #FFFFFF; font-size: 42px; font-weight: 900; font-family: 'Orbitron'; }
    
    /* Situación Actual con borde lateral blanco/dorado */
    .status-msg { 
        color: #FFFFFF; 
        font-style: italic; 
        font-size: 15px; 
...
with st.sidebar:
    ...
    st.markdown(f'<div class="sidebar-info">● SERVIDOR: OPERATIVO</div>', unsafe_allow_html=True)
    st.markdown(f'<div class="sidebar-info">● LATENCIA: 45ms</div>', unsafe_allow_html=True)
    st.markdown(f'<div class="sidebar-info">● EXCHANGE: MEXC CONNECTED</div>', unsafe_allow_html=True)
...
if data is not None:
    ...
    with c1: st.markdown(f'<div class="neon-panel"><div class="panel-header">PRECIO & EMA</div><div class="panel-content"><span class="price-main">${price:,.0f}</span></div></div>', unsafe_allow_html=True)
    with c2: st.markdown(f'<div class="neon-panel"><div class="panel-header">RSI ACTUAL</div><div class="panel-content"><span class="price-main">{rsi:.2f}</span></div></div>', unsafe_allow_html=True)
    with c3: st.markdown(f'<div class="neon-panel"><div class="panel-header">BILLETERA USDT</div><div class="panel-content"><span class="price-main" style="color:#DAA520;">${wallet:.2f}</span></div></div>', unsafe_allow_html=True)
    with c4: st.markdown(f'<div class="neon-panel"><div class="panel-header">GANANCIA</div><div class="panel-content"><span class="price-main" style="color:#00FF00;">${state["pnl_acumulado"]:.4f}</span></div></div>', unsafe_allow_html=True)

    log_msg = "Acechando entrada..."
    if not bot_encendido: log_msg = "SISTEMA EN PAUSA"
    else:
        # (Lógica de trading mantenida igual)
        if not state["in_position"]:
            ...
            st.markdown(f'<div style="text-align:center; color:#DAA520; font-size:14px; margin-bottom:10px;">BUY: ${promedio:,.0f} | TP: ${tp:,.0f}</div>', unsafe_allow_html=True)
    ...
    # SITUACIÓN ACTUAL CON TÍTULO EN BLANCO
    ...
    # HISTORIAL CON TÍTULO EN BLANCO Y GRILLA DE 5 COLUMNAS
    hist_html = '<div style="display: grid; grid-template-columns: 1fr 1fr 1fr 1fr 1fr; color: #DAA520; font-weight: bold; border-bottom: 1px solid #B8860B; padding-bottom: 8px; font-size: 13px;"><div>HORA</div><div>COMPRA</div><div>VENTA</div><div>NETO</div><div>PROFIT</div></div>'
    if state["history"]:
        for op in reversed(state["history"][-10:]):
            color_p = "#00FF00" if "-" not in op["Neto"] else "#FF4444"
            hist_html += f'<div style="display: grid; grid-template-columns: 1fr 1fr 1fr 1fr 1fr; padding: 10px 0; border-bottom: 1px solid #333; color: white; font-size: 13px;"><div>{op["Fecha"]}</div><div>{op["Entrada"]}</div><div>{op["Salida"]}</div><div style="color:{color_p};">{op["Neto"]}</div><div style="color:{color_p};">{op["Profit"]}</div></div>'
