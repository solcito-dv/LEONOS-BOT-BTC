...
if df_1m is not None and df_15m is not None:
    ...
    with c2: st.markdown(f'<div class="neon-panel"><div class="panel-header">ESTRATEGIA RSI</div><div class="panel-content"><span class="price-main">{rsi:.2f}</span><div style="color:#FFFF00; font-size:11px; font-weight:bold; margin-top:5px;">ABEJA < 40 | CAZA < 30</div></div></div>', unsafe_allow_html=True)
    with c3: st.markdown(f'<div class="neon-panel"><div class="panel-header">SALDO LIBRE</div><div class="panel-content"><span class="price-main" style="color:#FFFF00;">${cap_disponible:.3f}</span><div style="color:#FFFF00; font-size:12px; margin-top:5px;">CAPITAL: $10.0</div></div></div>', unsafe_allow_html=True)
    with c4: st.markdown(f'<div class="neon-panel"><div class="panel-header">GANANCIA TOTAL</div><div class="panel-content"><span class="price-main" style="color:#00FF00;">${state["pnl_acumulado"]:.4f}</span><div style="color:#00FF00; font-size:12px; margin-top:5px;">PNL ACUMULADO</div></div></div>', unsafe_allow_html=True)

    # --- 3 BURBUJAS CON PRECIOS ---
    if state["posiciones"]:
        ...
        for pos in state["posiciones"]:
            ...
            st.markdown(f'''
                <div class="burbuja b-compra">COMPRA: ${pos["precio"]:,.1f}</div>
                <div class="burbuja b-venta">VENTA: ${p_venta:,.1f}</div>
                <div class="burbuja b-sl">S. LOSS: ${p_sl:,.1f}</div><br>
            ''', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)

    # --- LÓGICA DE COMPRA ---
    log_msg = "Analizando mercado..." if bot_encendido else "SISTEMA EN PAUSA"
    if bot_encendido and len(state["posiciones"]) < 2:
        t_compra = None
        if rsi < 40 and price > ema9 and not any(p['tipo'] == "Abeja" for p in state["posiciones"]):
            t_compra = "Abeja"
        elif rsi < 30 and price > ema9 and radar_txt == "ALCISTA" and not any(p['tipo'] == "Cazadora" for p in state["posiciones"]):
            t_compra = "Cazadora"

        if t_compra:
            try:
                exchange.create_market_buy_order(SYMBOL, 4.95 / price)
                state["posiciones"].append({"precio": price, "monto": 4.95, "tipo": t_compra, "max_alc": price, "be_act": False})
                save_state(state)
                t_pct = target_ab if t_compra == "Abeja" else target_cz
                p_venta_t = price * (1 + t_pct/100)
                p_sl_t = price * (1 - 0.0120)
