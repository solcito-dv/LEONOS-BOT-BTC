...
# --- 2. CONEXIÓN GOOGLE SHEETS (SECRETS) ---
def conectar_gs():
    scope = ["https://spreadsheets.google.com/feeds", "https://www.googleapis.com/auth/drive"]
    creds_dict = st.secrets["gcp_service_account"]
    creds = ServiceAccountCredentials.from_json_keyfile_dict(creds_dict, scope)
    client = gspread.authorize(creds)
    return client.open("BTC_TRADING_DATA") # Tu nombre de archivo en Drive
...
# --- 3. ESTILOS (INTERFAZ ORIGINAL RECUPERADA) ---
...
try:
    ...
    with c2: st.markdown(f'<div class="neon-panel"><div class="panel-header">ESTRATEGIA RSI</div><div class="panel-content"><span class="price-main">{rsi:.2f}</span><div style="color:#FFFF00; font-size:11px; font-weight:bold;">ABEJA < 40 | CAZA < 35</div></div></div>', unsafe_allow_html=True)
    with c3: st.markdown(f'<div class="neon-panel"><div class="panel-header">SALDO LIBRE</div><div class="panel-content"><span class="price-main" style="color:#FFFF00;">${cap_disponible:.3f}</span><div style="color:#FFFF00; font-size:12px;">CAPITAL BASE: ${state["capital_asignado"]}</div></div></div>', unsafe_allow_html=True)
    ...
    # --- BURBUJAS DE OPERACIÓN ---
    if state["posiciones"]:
        st.markdown('<div style="text-align: center; margin-bottom: 20px;">', unsafe_allow_html=True)
        for pos in state["posiciones"]:
            t_obj = target_ab if pos['tipo'] == "Abeja" else target_cz
            st.markdown(f'<div class="burbuja b-entrada">[{pos["tipo"].upper()}] ${pos["precio"]:,.1f}</div><div class="burbuja b-venta">TARGET: {t_obj}%</div>', unsafe_allow_html=True)
    ...
    if bot_encendido:
        ...
        for pos in state["posiciones"]:
            ...
            vender = False
            if neta <= -1.20: vender = True # SL
            elif neta >= t_obj and caida_p <= -gap_v: vender = True # Pilla!
            elif neta >= (t_obj * 0.8) and caida_p <= -0.03: vender = True # Defensa
            ...
            if vender:
                try:
                    ...
                    v_row = [datetime.now().strftime('%H:%M:%S'), "BTC", pos['tipo'], pos['precio'], price, f"{neta:.2f}%", f"{profit:.4f}"]
