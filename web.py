...
def load_state():
    ...
    data_recuperada = {
        ...
        "history": [
            {"Fecha": "Manual", "Entrada": "Anterior", "Salida": "Venta", "Neto": "Profit", "Profit": "$0.0176"}
        ]
    }
...
if data is not None:
    ...
    # --- 7. HISTORIAL FINAL ---
    contenido_hist = '<div style="display: grid; grid-template-columns: 1fr 1fr 1fr 1fr 1fr; color: #FFFF00; font-weight: bold; border-bottom: 2px solid #DC143C; padding-bottom:8px; font-size:12px;"><div>HORA</div><div>COMPRA</div><div>VENTA</div><div>NETO</div><div>GANANCIA</div></div>'
    for op in reversed(state["history"][-10:]):
        color_neto = "#00FF00" if "-" not in op["Neto"] else "#FF0000"
        contenido_hist += f'<div style="display: grid; grid-template-columns: 1fr 1fr 1fr 1fr 1fr; padding: 10px 0; border-bottom: 1px solid #333; color: white; font-size: 13px;"><div>{op["Fecha"]}</div><div>{op["Entrada"]}</div><div>{op["Salida"]}</div><div style="color:{color_neto}; font-weight:bold;">{op["Neto"]}</div><div style="color:{color_neto};">{op["Profit"]}</div></div>'
