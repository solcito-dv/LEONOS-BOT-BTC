    ...
    if os.path.exists(STATE_FILE):
        try:
            with open(STATE_FILE, 'r') as f:
                data = json.load(f)
                # Forzamos que lea pnl_acumulado sin errores
                if "pnl_acumulado" not in data: data["pnl_acumulado"] = 0.0
                return data
...
# --- 2. ESTILOS (INTERFAZ ORIGINAL SIN RECUADROS EXTRAS) ---
...
st.markdown("""
    ...
    .b-info { background: #333; color: #FFFF00; border: 1px solid #FFFF00; }
    </style>
...
if df_1m is not None and df_15m is not None:
    ...
    # --- DASHBOARD (TAL CUAL LO TENÍAS) ---
    c1, c2, c3, c4 = st.columns(4)
    with c1: 
        st.markdown(f'''<div class="neon-panel"><div class="panel-header">PRECIO & EMA 9/200</div><div class="panel-content"><span class="price-main">${price:,.0f}</span>
        <div style="color:#FFFF00; font-size:12px; margin-top:5px;">EMA 9: ${ema9:,.0f} | EMA 200: ${ema200:,.0f}</div>
        <div style="font-size:11px; color:{radar_col}; font-weight:bold; margin-top:5px;">Radar 15m: {radar_txt}</div></div></div>''', unsafe_allow_html=True)
    ...
    # --- BURBUJAS DE POSICIÓN ---
    if state["posiciones"]:
        st.markdown('<div style="text-align: center; margin-bottom: 20px;">', unsafe_allow_html=True)
        for pos in state["posiciones"]:
            t_obj = target_ab if pos['tipo'] == "Abeja" else target_cz
            sl_actual = -1.20
            if pos.get('be_act', False): sl_actual = 0.0
            neta_cur = ((price - pos['precio']) / pos['precio']) * 100
            if neta_cur >= 0.30: sl_actual = 0.15
            st.markdown(f'<div class="burbuja b-entrada">[{pos["tipo"]}] ENTRADA: ${pos["precio"]:,.1f}</div><div class="burbuja b-info">TARGET: {t_obj}% | SL: {sl_actual}%</div><br>', unsafe_allow_html=True)
    ...
    if bot_encendido and len(state["posiciones"]) < 2:
        ...
        if t_compra:
            try:
                ...
                save_state(state)
                send_telegram_msg(f"🦁 *COMPRA {t_compra}* \n🔹 Símbolo: {SYMBOL} \n🔹 Entrada: ${price:,.2f} \n🔹 TP: {target_ab if t_compra=='Abeja' else target_cz}% | SL: -1.20%")
            except: pass
    ...
    # --- HISTORIAL (SIN RECUADROS EXTRAS) ---
