...
# --- 3. ESTILOS ---
st.set_page_config(page_title="LEONOS BTC | V34.4", layout="wide")
st.markdown("""
    <style>
...
st.markdown('<h2 style="font-family:Orbitron; color:#DC143C; margin-bottom:20px;">🦁 LEONOS BTC V34.4</h2>', unsafe_allow_html=True)
...
try:
    ...
    with c1: st.markdown(f'<div class="neon-panel dash-panel"><div class="panel-header">PRECIO & EMA 9</div><div class="panel-content"><span class="price-main">${price:,.0f}</span><div class="info-sub">EMA 9: ${ema9:,.0f}</div><div style="font-size:11px; color:{radar_col};">Radar 15m: {radar_txt}</div></div></div>', unsafe_allow_html=True)
    ...
    if bot_encendido:
        ...
        if len(state["posiciones"]) < 2 and cap_disponible >= monto_op:
            ...
            if rsi < 40 and price > ema9 and not ya_abeja: t_compra = "Abeja"
            elif rsi < 35 and radar_txt == "ALCISTA" and not ya_caza: t_compra = "Cazadora"
